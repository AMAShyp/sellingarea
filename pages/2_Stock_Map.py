...
class DeclareHandler(DatabaseManager):
    ...
    def get_shelf_entries(self, itemid):
        # Ensure itemid is Python int, not numpy.int64
        df = self.fetch_data("""
...
if not item is None:
    ...
    st.markdown(
        ...
        f"<span style='color:#C61C1C;font-weight:bold;'>Class:</span> <span style='color:#222'>{item['classcat']}</span><br>"
        f"<span style='color:#004CBB;font-weight:bold;'>Department:</span> <span style='color:#222'>{item['departmentcat']}</span><br>"
        f"<span style='color:#098A23;font-weight:bold;'>Section:</span> <span style='color:#222'>{item['sectioncat']}</span><br>"
