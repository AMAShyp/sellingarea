...
st.title("⏳ Near-Expiry by Remaining Stock (fast set-based)")
...
class ExpiryHandler(DatabaseManager):
    ...
    def get_near_expiry_allocations(self, near_days: int):
        ...
        One-shot SQL:
          ...
          3) compute near-expiry qty & batch counts
...
# final qty per item (for display context)
...
# join item meta for nicer names
...
# fast step 3: do it all in SQL
summary_df, detail_df = handler.get_near_expiry_allocations(near_days)

# attach names/barcodes to outputs
if not summary_df.empty and meta is not None and not meta.empty:
    summary_df = summary_df.merge(meta, on="itemid", how="left")
if not detail_df.empty and meta is not None and not meta.empty:
...
if summary_df.empty:
    ...
    st.dataframe(
        summary_df.rename(columns={
            "itemid":"Item ID","name":"Item Name","barcode":"Barcode",
            "near_expiry_qty":f"Near-Expiry ≤ {near_days}d","near_expiry_batches":"Affected Batches"
        })[["Item ID","Item Name","Barcode",f"Near-Expiry ≤ {near_days}d","near_expiry_batches"]],
        use_container_width=True, hide_index=True
    )
...
if detail_df.empty:
    ...
    # format days_left nicely
    out = detail_df.copy()
    st.dataframe(
        out.rename(columns={
            "itemid":"Item ID","name":"Item Name","barcode":"Barcode",
            "batchid":"Batch ID","batch_qty":"Batch Qty","used_from_batch":"Used From Batch",
            "expirationdate":"Expiry Date","received_ts":"Received At","days_left":"Days Left"
        })[["Item ID","Item Name","Barcode","Batch ID","Batch Qty","Used From Batch","Expiry Date","Days Left","Received At"]],
        use_container_width=True, hide_index=True
    )
