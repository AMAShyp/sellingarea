...
def alerts_tab() -> None:
    ...
    tab1, tab2 = st.tabs(
        ["⚠️ Low Stock Items", "⏰ Near Expiry Items"]
    )
    ...
    with tab1:
        ...
        if shelf_qty_df.empty:
            ...
            if alerts_df.empty:
                st.success(
                    "✅ All items meet or exceed their shelf threshold."
                )
            else:
    ...
    with tab2:
        ...
        item_df = shelf_handler.fetch_data(
            "SELECT itemid, shelflife FROM item"
        )
        shelf_df = shelf_df.merge(item_df, on="itemid", how="left")

        subtab_days, subtab_percent = st.tabs(
            ["📅 Days-Based", "📐 Shelf Life %"]
        )
        ...
        with subtab_days:
            ...
            near_expiry_df = shelf_df[
                shelf_df["days_left"] <= green_days
            ].copy()
